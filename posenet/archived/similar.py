...
def findDeviations(original_path, new_path):
    original_kps, original_show = pose(original_path)
    new_kps, new_show = pose(new_path)

    original_values = generate_values(original_kps)
    new_values = generate_values(new_kps)
    deviations, angle_diffs = matching(original_values, new_values)

    draw_deviations(new_show, new_kps, connected_points, deviations)
    cv.imshow("deviations", new_show)
    cv.waitKey()
    return deviations, angle_diffs
...
devs, adiff = findDeviations("test7.png", "test6.jpg")
print (devs)
print (adiff)
