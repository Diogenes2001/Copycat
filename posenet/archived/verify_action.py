...
# import numpy as np
# import math
# from similar import findDeviations

# def keypoints_to_vector(keypoints):
#     out = np.array([])
#     for kp in keypoints:
#         out.append(kp['position']['y'])
#         out.append(kp['position']['x'])
#     return out

# def dist(v1, v2):
#     return math.sqrt(2 * (1-skl.metrics.pairwise.cosine_similarity(v1, v2)))

# # devs = findDeviations("test7.png", "test8.jpg")
...
#             # dists = [devs(frame, new_frame) for new_frame in frames]
...
def verify_actions(past_actions, new_actions):
	for past in past_actions:
		for new in new_actions:
			devs, dists = findDeviations(past, new)
